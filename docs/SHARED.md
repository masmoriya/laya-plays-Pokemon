...
|---|---|---|---|---|
| `Laya-Plays-Pokemon` | Pokémon Red on PyBoy, Laya picks at branches, code owns the goal stack and pathfinding | thin (two 0-1 star repos, one is battles only) | decisions/sec and $/hour, plus a running Brier score | live probability bars over the emulator |
