...
class AudioSink:
    ...
    def __init__(self, sample_rate: int = AUDIO_SAMPLE_RATE):
        ...
        self._start_bytes = int(sample_rate * AUDIO_START_BUFFER_SECONDS) * AUDIO_CHANNELS
    ...
    def set_speed(self, speed: float) -> None:
        """Silence fast-forward modes instead of dropping samples as static."""
        realtime = speed == 1.0
    ...
    def _apply_volume(self) -> None:
        if self.channel is not None:
            self.channel.set_volume(0.0 if self.muted or not self._realtime else 1.0)

    def _pump(self) -> None:
        if len(self._pending) < self._chunk_bytes:
            return
        if not self.channel.get_busy() and len(self._pending) < self._start_bytes:
            return
    ...
    def flush(self) -> None:
        """Discard stale queued samples without disabling later playback."""
        if self.channel is not None:
            self.channel.stop()
        self._pending.clear()
